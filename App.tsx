...
import { AppState, DecisionType, SurveyData, APIError } from './types';
...
const App: React.FC = () => {
  ...
  const { activeTheme, setActiveTheme, getTagColor } = useTheme();
  const { archive, isSyncing, cloudConnected, saveSimulation, deleteSimulation } = useArchive();

  console.log('Hooks initialized - theme:', activeTheme, 'archive length:', archive.length);

  const handleStartResearch = useCallback(async (decision: string, tags: DecisionType[]) => {
    setState(prev => ({ 
      ...prev, 
      loading: true, 
      stage: 'researching',
    ...
    // Set theme based on primary tag
    if (tags.length > 0) {
      setActiveTheme(tags[0]);
    }
    ...
    try {
      ...
      const errorMessage = error instanceof APIError 
        ? `${error.service?.toUpperCase() || 'API'} Error: ${error.message}`
  ...
  const loadFromArchive = useCallback((item: SurveyData) => {
    setState(prev => ({
      ...prev,
      stage: 'results',
      surveyData: item,
  ...
  const clearArchive = useCallback(async () => {
    if (confirm("Clear your simulation archive? This will affect your cloud and local storage.")) {
      try {
        await Promise.all(archive.map(item => deleteSimulation(item.id)));
      } catch (error) {
        console.error('Failed to clear archive:', error);
      }
    }
  }, [archive, deleteSimulation]);
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen selection:bg-indigo-100 flex flex-col relative overflow-hidden">
        ...
        <nav className="p-6 flex items-center justify-between no-print sticky top-0 bg-white/80 backdrop-blur-md z-40 border-b border-slate-100 shadow-sm">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-500">
            <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full border shadow-sm transition-all ${
              ...
              {cloudConnected ? `${archive.length} Cloud Saved` : 'Offline Mode (Local Storage)'}
            </div>
        ...
        {state.loading && state.stage === 'researching' && (
          ...
          {state.stage === 'setup' ? (
            <div className="max-w-6xl mx-auto flex flex-col items-center space-y-32">
              <div className="text-center space-y-6 max-w-4xl">
                ...
                {state.error && (
                  <div className="mx-auto max-w-md p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm font-medium">
                    {state.error}
                  </div>
                )}
