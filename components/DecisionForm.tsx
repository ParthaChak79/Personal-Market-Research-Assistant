...
const DECISION_TYPES: { type: DecisionType; icon: React.ReactNode; color: string }[] = [
  { type: 'Personal', icon: <User size={18} />, color: 'bg-blue-50 text-blue-600 border-blue-200' },
  { type: 'Career', icon: <Briefcase size={18} />, color: 'bg-emerald-50 text-emerald-600 border-emerald-200' },
  { type: 'Business', icon: <TrendingUp size={18} />, color: 'bg-purple-50 text-purple-600 border-purple-200' },
  { type: 'Investment', icon: <Sparkles size={18} />, color: 'bg-amber-50 text-amber-600 border-amber-200' },
  { type: 'Lifestyle', icon: <Heart size={18} />, color: 'bg-rose-50 text-rose-600 border-rose-200' },
];

export const DecisionForm: React.FC<DecisionFormProps> = ({ onSubmit, loading }) => {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-4 relative">
          ...
          <div className="relative group">
            <textarea
              ...
              className="w-full h-40 p-6 text-xl border border-slate-200 rounded-[2.5rem] focus:ring-8 focus:ring-indigo-100 focus:border-indigo-400 transition-all outline-none resize-none glass-panel shadow-sm leading-relaxed"
              disabled={loading}
            />
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {DECISION_TYPES.map(({ type, icon, color }) => (
              <button
                key={type}
                type="button"
